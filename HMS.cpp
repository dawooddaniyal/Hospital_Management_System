...
public:
    ...
    void set() {
        ...
        if (choice == 1) {
            ...
            int toAdd = 0;
            cin >> toAdd;
            if (toAdd <= 0) { cout << "  [Error] Invalid number." << endl; return; }
            ...
            cin >> newCharge;
            if (newCharge < 0) { cout << "  [Error] Charge cannot be negative." << endl; return; }
    ...
    void removeBed() {
        ...
        if (foundIdx == -1)          { cout << "  [Error] Bed ID not found."          << endl; return; }
        if (beds[foundIdx].reserved) { cout << "  [Error] Cannot remove reserved bed." << endl; return; }
...
public:
    ...
    void set() {
        ...
        while (more == 'y' || more == 'Y') {
            ...
            cin >> m.pricePerItem;
            if (m.pricePerItem < 0) { cout << "  [Error] Price cannot be negative." << endl; continue; }
            cout << "  Enter stock quantity: ";
            cin >> m.stock;
            if (m.stock < 0) { cout << "  [Error] Stock cannot be negative." << endl; continue; }
    ...
    void get() {
        ...
        if (medics.empty()) { cout << "  No medicines in inventory." << endl; return; }
        printLine();
        for (int i = 0; i < (int)medics.size(); i++) {
    ...
    void removeItem() {
        ...
        if (idx == -1) { cout << "  [Error] Medicine ID not found." << endl; return; }
...
public:
    ...
    void displayMedication() {
        ...
        if (meds.empty()) { cout << "  No medications recorded." << endl; return; }
        printLine();
    ...
    void generateBill() {
        if (!isAdmitted) { cout << "  [Error] Patient has no admission record." << endl; return; }
...
public:
    ...
    void dischargePatient() {
        ...
        if (!p)                  { cout << "  [Error] Patient not found."          << endl; return; }
        if (p->getIsDischarged()){ cout << "  [Error] Patient already discharged." << endl; return; }
    ...
    void removePatient() {
        ...
        if (idx == -1) { cout << "  [Error] Patient not found." << endl; return; }
    ...
    void managePatient() {
        ...
        if (!findByCnic(cnic)) { cout << "  [Error] Patient not found." << endl; return; }

        int choice = 0;
        while (choice != 5) {
            // Re-fetch pointer each iteration in case vector reallocated
            Patient *p = findByCnic(cnic);
            if (!p) { cout << "  [Error] Patient record no longer exists." << endl; return; }
...
private:
    ...
    string getPassword() {
        ...
        while (true) {
            ch = _getch();
            if (ch == '\r' || ch == '\n') break;           // Enter — done
            else if (ch == 0 || ch == 224) _getch();       // arrow keys — ignore
